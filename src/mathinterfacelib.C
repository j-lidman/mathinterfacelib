...
namespace MathInterfaceLib {
   ...
   void Value_Matrix::addElementToLastRow(Value *vt) {
      if(vt == NULL)
         throw MathInterfacelibException("Invalid element");
      if(mat.size() == 0)
         throw MathInterfacelibException("Matrix has no rows");
   ...
   ENGINE_TYPE getStateType(State *s) {
      if(!s->good())
         throw MathInterfacelibException("Engine in erronous state!");
      #if !defined(MATHINTERFACELIB_PREFER_STATIC_ERRORS) || (defined(MATHINTERFACELIB_PREFER_STATIC_ERRORS) && defined(HAS_MATLAB))
         ...
         return ENGINE_OFFLINE;
          ...
          throw MathInterfacelibException("Unknown engine type");
   }
   ...
   Value *createValTree(Function::Argument *arg) {
      ...
      if(argArr != NULL)
         ...
         switch(argElem->t) {
          case TYPE_UNKNOWN:   throw MathInterfacelibException("Can't create element of type unknown");
          ...
          default:            throw MathInterfacelibException("Unhandled cast in createValTree::element");
         }
      } else
         throw MathInterfacelibException("Unhandled cast in createValTree()");
   }
   ...
   State *init_engine(ENGINE_TYPE e, std::string path, bool enableSharedFunc) {
      //Create application specific engine
        switch(e) {
         ...
         case ENGINE_MATLAB:
           {
               //Initialize shared memory
                 ...
                 if(enableSharedFunc) {
                    ...
                    shm = new shared_memory_object(open_or_create, MATHINTERFACELIB_SHARED_MEM_NAME, read_write);
                    ...
                    if(!data->regionInUse.try_lock())
                       throw MathInterfacelibException("MathInterfaceLib library already open");
                 }
         ...
         default:
            throw MathInterfacelibException("Unable to initialize engine: unknown type");
        }
   }
   void finit_engine(State *s) {
      ...
      throw MathInterfacelibException("Engine has not been initialized or invalid type");
   }
   ...
   //Function handling
     Value *getArg(Value *in, size_t row, size_t col) {
        ...
        throw MathInterfacelibException("Value tree is not a matrix");
     }
     Value *getArg(Value *in, size_t index) {
        ...
        if(vtStruct != NULL)
           ...
           throw MathInterfacelibException("Value tree is not a non-terminal");
     }

     template<> size_t getValue<size_t>(Value *in) {
        Value_Scalar<size_t> *t = dynamic_cast<Value_Scalar<size_t> *>(in);
        if(t == NULL)
           throw MathInterfacelibException("Value tree is not a terminal");
        return t->val;
     }
