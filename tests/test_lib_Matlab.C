...
template<class STATE_T> int basic_statetest(STATE_T *s, bool expectExceptionOnRead) {
   ...
   //Read/write iterator
     ...
     //Double
       s->writeVar("arrDbl", 1, 10, dblArr);
       readResults = true;
       try {
           dstDblArr = s->readDoubleArr("arrDbl", szDims);
       } catch(MathInterfacelibException e) {
           if(!expectExceptionOnRead)
              throw e;
           else
       ...
       s->writeVar("vecDbl", 1, 10, FWD_IT_T<double>(vecDbl.begin()), FWD_IT_T<double>(vecDbl.end()));
       readResults = true;
       try {
           dstDblArr = s->readDoubleArr("vecDbl", szDims);
       } catch(MathInterfacelibException e) {
           if(!expectExceptionOnRead)
              throw e;
           else
       ...
       delete[] dblArr;
     //Int
       s->writeVar("arrInt", 1, 10, intArr);
       readResults = true;
       try {
           dstIntArr = s->readIntArr("arrInt", szDims);
       } catch(MathInterfacelibException e) {
           if(!expectExceptionOnRead)
              throw e;
           else
       ...
       s->writeVar("vecInt", 1, 10, FWD_IT_T<size_t>(vecInt.begin()), FWD_IT_T<size_t>(vecInt.end()));
       readResults = true;
       try {
           dstIntArr = s->readIntArr("vecInt", szDims);
       } catch(MathInterfacelibException e) {
           if(!expectExceptionOnRead)
              throw e;
           else
       ...
       delete[] intArr;
   //Read/write string
     s->writeVar("teststr", "ABCDEFGHIJKLMNOPQRSTUVWXYZ");
     std::string str;
     readResults = true;
     try {
         str = s->readStrVar("teststr");
     } catch(MathInterfacelibException e) {
         if(!expectExceptionOnRead)
            throw e;  
         else
